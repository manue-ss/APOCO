...
/**
 * Implementación de una lista enlazada simple genérica. Permite almacenar una
 * secuencia ordenada de elementos de tipo {@code T}. Soporta operaciones
 * básicas de inserción, eliminación, búsqueda y clonación. Mantiene referencias
 * a la cabeza y la cola para optimizar algunas operaciones. Puede ser ordenada
 * usando el patrón Strategy si los elementos son {@link Comparable}.
 *
...
public class ListaEnlazadaSimple<T> {
     ...
     * está vacía. Optimiza inserción al final.
     */
    private Nodo<T> cola;
    /**
    ...
    /**
     * Obtiene el nodo cabeza de la lista. Utilizado internamente por algunas
     ...
     * estrategias de ordenación que reestructuran la lista. Este método también
     * se encarga de actualizar la referencia a la cola.
     *
     ...
     * tiempo constante O(1) gracias a la referencia {@code cola}.
     *
     * @param dato El dato a insertar.
     */
    ...
    /**
     * Alias conveniente para {@link #insertarAlFinal(Object)}. Agrega un
     ...
     * búsqueda es O(n), la inserción es O(1) una vez encontrado el nodo.
     *
     * @param datoExistente El dato del nodo después del cual se realizará la
     *                      inserción. Se compara usando {@code equals()}.
     * @param datoNuevo     El dato del nuevo nodo a insertar.
     *
     * @return {@code true} si la inserción se realizó con éxito, {@code false}
     *         si {@code datoExistente} no fue encontrado.
     */
     ...
     * encuentra (y no es la cabeza), la lista no se modifica. La búsqueda es
     * O(n), la inserción es O(1).
     *
     * @param datoExistente El dato del nodo antes del cual se realizará la
     *                      inserción. Se compara usando {@code equals()}.
     * @param datoNuevo     El dato del nuevo nodo a insertar.
     *
     * @return {@code true} si la inserción se realizó con éxito, {@code false}
     *         si {@code datoExistente} no fue encontrado (y no era la cabeza).
     */
    ...
    /**
     * Elimina y devuelve el elemento que se encuentra al principio de la lista
     * (la cabeza). Operación de tiempo constante O(1).
     *
    ...
    /**
     * Elimina y devuelve el elemento que se encuentra al final de la lista (la
     * cola). Requiere recorrer la lista para encontrar el penúltimo nodo, por
     * lo que es O(n).
     *
     * @return El dato del elemento eliminado.
     *
     ...
     *
     * @param datoExistente El dato del nodo después del cual se eliminará. Se
     *                      compara usando {@code equals()}.
     *
     ...
     * la primera ocurrencia del nodo que contiene {@code datoExistente}. No se
     * puede eliminar antes de la cabeza. Requiere al menos dos nodos en la
     * lista para poder eliminar algo "antes" de un nodo que no sea la cabeza.
     * La búsqueda es O(n).
     *
     * @param datoExistente El dato del nodo antes del cual se eliminará. Se
     *                      compara usando {@code equals()}.
     *
     * @return El dato del nodo eliminado, o {@code null} si no se pudo realizar
     *         la eliminación.
    ...
    // --- Otras Utilidades ---
    /**
     * Elimina todos los elementos de la lista, dejándola en su estado inicial
     * (vacía). Las referencias a cabeza y cola se anulan, y el tamaño se pone a
     * 0. Los nodos anteriores serán elegibles para recolección de basura.
     */
     ...
     * crean nuevos nodos, pero estos contienen referencias a los mismos objetos
     * de datos que la lista original. Las modificaciones a los objetos de datos
     * se reflejarán en ambas listas. Modificaciones a la estructura de una
     * lista (añadir/quitar nodos) no afectan a la otra.
     *
     * @return Una nueva instancia de {@code ListaEnlazadaSimple} con los mismos
     *         datos.
     */
    ...
    // --- Búsqueda ---
    /**
     * Comprueba si la lista contiene al menos una ocurrencia del elemento
     * especificado. Utiliza {@code equals()} para la comparación. La búsqueda
     * es O(n).
     *
     * @param dato El dato a buscar en la lista.
     *
     * @return {@code true} si el dato se encuentra en la lista, {@code false}
     *         en caso contrario.
     */
     ...
     * La lista se modifica "in situ". Requiere que el tipo de los elementos
     * {@code T} implemente {@link Comparable}.
     *
     * @param <U>        El tipo de los elementos (debe ser Comparable). Se usa
     *                   para asegurar type safety.
     * @param estrategia La instancia del algoritmo de ordenación
     *                   ({@link EstrategiaOrdenamiento}) a aplicar.
     *
     * @throws IllegalStateException si los elementos de la lista no son
     *                               {@code Comparable}.
    ...
    // --- Iterador ---
    /**
     * Devuelve un iterador simple para recorrer los elementos de la lista desde
     * la cabeza hasta la cola. Este iterador no soporta la operación
     * {@code remove()}.
     *
     ...
     * Permite recorrer la lista secuencialmente usando los métodos
     * {@code hasNext()} y {@code next()}. No implementa {@code remove()}. Es
     * una clase estática anidada.
     *
     * @param <I> El tipo de elementos sobre los que itera (debe coincidir con T
     *            de la lista).
     */
    public static class Iterador<I> {
         ...
         *
         * @return {@code true} si {@code actual} no es {@code null},
         *         {@code false} en caso contrario.
         */
        ...
        /**
         * Devuelve el dato del nodo actual y avanza el iterador al siguiente
         * nodo.
         *
         * @return El dato del nodo actual.
         *
         * @throws NoSuchElementException si se llama cuando
         *                                {@link #tieneSiguiente()} es
         *                                {@code false}.
         */
        public I siguiente() {
            if (!tieneSiguiente()) {
     ...
     *
     * @return El {@link Nodo} que contiene el dato, o {@code null} si no se
     *         encuentra.
     */
    ...
    /**
     * Busca el nodo que se encuentra inmediatamente *antes* del primer nodo que
     * contiene {@code datoBusqueda}. Utiliza {@code Objects.equals()} para
     * manejar {@code null} de forma segura.
     *
     * @param datoBusqueda El dato contenido en el nodo objetivo (el nodo
     *                     *después* del que buscamos).
     *
     * @return El nodo predecesor al nodo que contiene {@code datoBusqueda}, o
     *         {@code null} si {@code datoBusqueda} está en la cabeza, no se
     *         encuentra, o la lista tiene menos de 2 elementos.
     */
