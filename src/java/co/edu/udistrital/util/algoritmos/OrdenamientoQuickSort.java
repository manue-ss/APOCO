...
import co.edu.udistrital.util.listas.Nodo;
 ...
 * Quick Sort (ordenación rápida). Es un algoritmo eficiente de tipo "divide y
 * vencerás". Selecciona un elemento como pivote y particiona la lista de forma
 * que los elementos menores que el pivote queden a un lado y los mayores al
 * otro, luego ordena recursivamente las sublistas.
 *
 * <p>
 * <b>Nota Importante:</b> Esta implementación específica utiliza el
 * <strong>último elemento</strong>
 * de la (sub)lista como pivote e intercambia los <strong>datos</strong>
 * contenidos en los nodos, no los nodos en sí. Por lo tanto, requiere que la
 * clase {@link Nodo} posea un método {@code setDato(T)}.</p>
 * <p>
 * QuickSort en listas enlazadas puede ser menos eficiente que en arrays debido
 * a la dificultad de acceso aleatorio y la complejidad de la partición.</p>
 *
 * <p>
 * Complejidad Temporal: O(n log n) en promedio, O(n^2) en el peor caso (ej.
 * lista ya ordenada).</p>
 * <p>
 * Complejidad Espacial: O(log n) en promedio (pila recursión), O(n) en peor
 * caso.</p>
 *
...
public class OrdenamientoQuickSort<T extends Comparable<T>> implements EstrategiaOrdenamiento<T> {
     ...
     * nodo como pivote y realizando intercambio de datos.
     *
     * @param lista La lista {@link ListaEnlazadaSimple} a ordenar. No debe ser
     *              null.
     *
     * @throws NullPointerException          si {@code lista} es null.
     * @throws ClassCastException            si los elementos no son
     *                                       {@code Comparable}.
     * @throws UnsupportedOperationException si {@link Nodo#setDato(Object)} no
    ...
    /**
     * Particiona la (sub)lista (desde {@code cabeza} hasta {@code cola}) usando
     * el dato del nodo {@code cola} como pivote. Reorganiza los
     * <strong>datos</strong> de los nodos para que los menores queden antes del
     * pivote y los mayores después.
     *
     * @param cabeza El primer nodo de la (sub)lista a particionar.
     * @param cola   El último nodo (pivote) de la (sub)lista a particionar.
     *
     * @return Un array de
     *         {@link Nodo}: {@code [nodoPivoteFinal, nodoAntesDelPivoteFinal]}.
     *         El {@code nodoAntesDelPivoteFinal} puede ser {@code null} si el
     *         pivote termina en la cabeza.
     *
     * @throws UnsupportedOperationException si {@link Nodo#setDato(Object)} no
     *                                       está disponible (implícito).
     */
