 ...
 * Merge Sort (ordenación por mezcla). Es un algoritmo eficiente de tipo "divide
 * y vencerás", particularmente adecuado para listas enlazadas ya que no
 * requiere acceso aleatorio. Divide la lista recursivamente, ordena las
 * sublistas y luego las fusiona (merge) de manera ordenada.
 *
 * <p>
 ...
 *
 * <p>
 * Complejidad Temporal: O(n log n) en todos los casos (peor, promedio,
 * mejor).</p>
 * <p>
 * Complejidad Espacial: O(log n) debido a la pila de recursión (puede ser O(n)
 * en algunas implementaciones iterativas).</p>
 *
...
public class OrdenamientoMerge<T extends Comparable<T>> implements EstrategiaOrdenamiento<T> {
    ...
    /**
     * Función recursiva principal de Merge Sort. Divide la (sub)lista, ordena
     * recursivamente las mitades y las fusiona.
     *
    ...
    /**
     * Encuentra el nodo medio de una lista enlazada dada su cabeza. Si la lista
     * tiene un número par de elementos, devuelve el nodo final de la primera
     * mitad. Utiliza la técnica de punteros lento y rápido.
     *
    ...
    /**
     * Fusiona (merge) dos listas enlazadas, {@code izquierda} y
     * {@code derecha}, que ya se encuentran ordenadas. Devuelve una nueva lista
     * enlazada ordenada que contiene todos los elementos de ambas listas
     * originales.
     *
