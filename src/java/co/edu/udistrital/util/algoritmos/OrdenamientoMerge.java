 ...
 * Es un algoritmo eficiente de tipo "divide y vencerás", particularmente
 * adecuado para listas enlazadas
 * ya que no requiere acceso aleatorio. Divide la lista recursivamente, ordena
 * las sublistas y
 ...
 *
 * <p>
 * Complejidad Temporal: O(n log n) en todos los casos (peor, promedio, mejor).
 * </p>
 * <p>
 * Complejidad Espacial: O(log n) debido a la pila de recursión (puede ser O(n)
 * en algunas implementaciones iterativas).
 * </p>
 *
...
public class OrdenamientoMerge<T extends Comparable<T>> implements EstrategiaOrdenamiento<T> {
     ...
     *
     * @param cabeza El {@link NodoDoble} cabeza de la (sub)lista a ordenar.
     * @return El {@link NodoDoble} cabeza de la (sub)lista ya ordenada.
     */
    private NodoDoble<T> mergeSortRecursivo(NodoDoble<T> cabeza) {
        ...
        NodoDoble<T> segundaMitadCabeza = medio.getSiguiente();
        ...
        NodoDoble<T> derechaOrdenada = mergeSortRecursivo(segundaMitadCabeza);
     ...
     * Si la lista tiene un número par de elementos, devuelve el nodo final de la
     * primera mitad.
     * Utiliza la técnica de punteros lento y rápido.
     *
     * @param cabeza El {@link NodoDoble} inicial de la lista.
     * @return El {@link NodoDoble} que representa el final de la primera mitad.
     */
    ...
    private NodoDoble<T> fusionar(NodoDoble<T> izquierda, NodoDoble<T> derecha) {
        ...
        NodoDoble<T> cabezaResultado;

        // Comparar los primeros nodos de cada lista para decidir la cabeza del
        // resultado
        if (izquierda.getDato().compareTo(derecha.getDato()) <= 0) {
            // El elemento de la izquierda es menor o igual
            cabezaResultado = izquierda;
            // El siguiente del resultado será la fusión del resto de la izquierda con toda
            // la derecha
            ...
            // El siguiente del resultado será la fusión de toda la izquierda con el resto
            // de la derecha
